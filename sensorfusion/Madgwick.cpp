...
#define sampleFreq  125.0f
#define betaDef   0.02f
...
void MadgwickAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) {
    ...
    if (!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
        ...
        hx = mx * q0q0 - _2q0my * q3 + _2q0mz * q2 + mx * q1q1 + _2q1 * my * q2 + _2q1 * mz * q3 - mx * q2q2 - mx *
             q3q3;
        hy = _2q0mx * q3 + my * q0q0 - _2q0mz * q1 + _2q1mx * q2 - my * q1q1 + my * q2q2 + _2q2 * mz * q3 - my * q3q3;
        _2bx = sqrt(hx * hx + hy * hy);
        _2bz = -_2q0mx * q2 + _2q0my * q1 + mz * q0q0 + _2q1mx * q3 - mz * q1q1 + _2q2 * my * q3 - mz * q2q2 + mz *
               q3q3;
        _4bx = 2.0f * _2bx;
        _4bz = 2.0f * _2bz;
...
void MadgwickAHRSupdateIMU(float gx, float gy, float gz, float ax, float ay, float az) {
    ...
    if (!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
        // Normalise accelerometer measurement
        recipNorm = invSqrt(ax * ax + ay * ay + az * az);
        //crecipNorm = nativeinvSqrt(ax * ax + ay * ay + az * az);
        ax *= recipNorm;
        ay *= recipNorm;
        az *= recipNorm;
        ...
        recipNorm = invSqrt(s0 * s0 + s1 * s1 + s2 * s2 + s3 * s3); // normalise step magnitude
        //recipNorm = nativeinvSqrt(s0 * s0 + s1 * s1 + s2 * s2 + s3 * s3); // normalise step magnitude
        s0 *= recipNorm;
        s1 *= recipNorm;
        s2 *= recipNorm;
        s3 *= recipNorm;
    ...
    // Normalise quaternion
    recipNorm = invSqrt(q0 * q0 + q1 * q1 + q2 * q2 + q3 * q3);
    // recipNorm = nativeinvSqrt(q0 * q0 + q1 * q1 + q2 * q2 + q3 * q3);
    q0 *= recipNorm;
    q1 *= recipNorm;
    q2 *= recipNorm;
    q3 *= recipNorm;
...
void eulerAngles(float *euler_angles) {
    ...
    //Itt eredetileg roll = 0, pitch = 0 de a gimbal lock és imu helyzete miatt megfordítom
    euler_angles[0] = roll;
...
void eulerAnglesRPswap(float *euler_angles) {
    //Swapped roll and pitch calculation
