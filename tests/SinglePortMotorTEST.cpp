...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        for (const auto &props: motor_props) {
            QThread *controller_thread = QThread::create([&redis, props, n_motors]() {
                try {
                    ...
                    exoskeleton::redis_tools::log(redis, props.name, e.what(),
                                                  exoskeleton::redis_tools::LogLevel::error);
                } catch (...) {
                    std::cerr << props.name << " [ERROR] Exception" << std::endl;
                    exoskeleton::redis_tools::log(redis, props.name, "Unknown exception",
                                                  exoskeleton::redis_tools::LogLevel::error);
                }
            });
        ...
        // IMU controller thread, same pattern
        QThread *imu_qthread = QThread::create([uri] {
            try {
                MCP2221 mcp2221;
                ...
                std::cerr << "[IMU] Exception: " << e.what() << '\n';
            } catch (...) {
                std::cerr << "[IMU] Unknown exception\n";
            }
        });
        threads.push_back(imu_qthread);
