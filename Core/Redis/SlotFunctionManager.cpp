...
bool SlotFunctionManager::loadDefaultFunction(QVector<double>& outValues)
{
    ...
    // Convert control points to 360-element array using linear interpolation
    outValues.clear();
    outValues.reserve(360);
...
bool SlotFunctionManager::uploadFunctionToMotor(int address, int slot, const QVector<double>& values)
{
    ...
    if (values.size() != 360) {
        qWarning() << "[SlotFunctionManager] Invalid values size:" << values.size() << "(expected 360)";
        return false;
    }
