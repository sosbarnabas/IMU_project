...
void Facade::set_user_params(const exoskeleton::settings::UserParams& params) {
    // Convert UserParams to map
    auto m = params.to_map();
    
    // Save to Redis as hash
    _redis.del("conf:user");  // Clear existing
...
bool Facade::initialize_env_from_file(const std::string& env_path, bool overwrite) {
    std::cout << "[RedisFacade] initialize_env_from_file called with path: " << env_path 
              << ", overwrite: " << (overwrite ? "true" : "false") << std::endl;
    try {
        int uploaded = EnvLoader::upload_to_redis(_redis, env_path, overwrite);
        std::cout << "[RedisFacade] EnvLoader::upload_to_redis returned: " << uploaded << std::endl;
        if (uploaded > 0) {
            std::cout << "[RedisFacade] Initialized conf:env with " << uploaded << " keys from " << env_path << std::endl;
            return true;
        } else {
            std::cout << "[RedisFacade] No keys uploaded (returned 0)" << std::endl;
            return false;
        }
    } catch (const std::exception& e) {
        std::cerr << "[RedisFacade] EXCEPTION caught: " << e.what() << std::endl;
        return false;
    }
}
...
void Facade::initialize_run_addrs_from_env() {
    try {
        ...
        // Build run:addrs from motor serial numbers
        ...
        auto add_motor = [&](const std::string& motor_name, const std::string& serial) {
            if (!serial.empty()) {
                ...
                std::cout << "[RedisFacade] Adding motor: " << motor_name << " = " << value << std::endl;
                address++;
            }
        };
        ...
        if (motor_mappings.empty()) {
            std::cerr << "[RedisFacade] No motors found in conf:env" << std::endl;
            return;
        }
        
        // Clear existing run:addrs and upload new mappings
        _redis.del("run:addrs");
        _redis.hset("run:addrs", motor_mappings.begin(), motor_mappings.end());
        
        std::cout << "[RedisFacade] Initialized run:addrs with " << motor_mappings.size() << " motors" << std::endl;
        
    } catch (const std::exception& e) {
        std::cerr << "[RedisFacade] Failed to initialize run:addrs: " << e.what() << std::endl;
    }
}
