    ...
    , m_functionManager(new SlotFunctionManager(this))
{
    ...
    try {
        qDebug() << "[TelemetryController] Creating initial RedisFacade with URI:" << m_redisUri;
        m_redisFacade = std::make_unique<exoskeleton::redis::Facade>(m_redisUri.toStdString());
        
        // Try to initialize conf:env from .env file if it doesn't exist
        // Use relative path from workspace root or default Windows path
        QString envPath = "C:/Users/orban/CLionProjects/ExoGui/.env";
        ...
        qDebug() << "[TelemetryController] Checking for .env file at:" << envPath;
        if (m_redisFacade->initialize_env_from_file(envPath.toStdString(), false)) {
            qDebug() << "[TelemetryController] ✓ conf:env initialized from .env file";
        } else {
            qDebug() << "[TelemetryController] conf:env already exists or no .env file found";
        }
        
        // Check if run:addrs already exists in Redis
        sw::redis::Redis redis(m_redisUri.toStdString());
        bool runAddrsExists = redis.exists("run:addrs");
        
        if (runAddrsExists) {
            qDebug() << "[TelemetryController] run:addrs already exists, using existing configuration";
        } else {
            qDebug() << "[TelemetryController] run:addrs doesn't exist, will wait for StartupConfigDialog or manual configuration";
            // DON'T auto-initialize - let the user choose via StartupConfigDialog
        }
        
        // Load settings from Redis (this won't fail even if run:addrs is empty)
        loadRedisSettings();
        
        qDebug() << "[TelemetryController] ✓ Initial configuration loaded successfully";
    } catch (const std::exception& e) {
        qWarning() << "[TelemetryController] Failed to initialize RedisFacade:" << e.what();
    }
    ...
    // Trigger immediate refresh to ensure GUI gets updated
    if (m_client) {
        qDebug() << "[TelemetryController] Triggering initial Redis data refresh...";
        QTimer::singleShot(500, this, [this]() {
            if (m_client) {
                m_client->refreshFromRedis();
            }
        });
    }
}
...
void TelemetryController::handleSampleReceived(int addr, const TelemetrySample &sample)
{
    ...
    if (m_home && sample.enabled) {
        ...
        // Add to pending updates instead of immediate update (include torque)
        m_pendingUpdates.append({motorName, timestamp, static_cast<double>(sample.position), static_cast<double>(sample.torque)});
    }
}
...
void TelemetryController::processBatchedUpdates()
{
    ...
    for (const auto& update : m_pendingUpdates) {
        m_home->updateMotorData(update.motorName, update.timestamp, update.position, update.torque);
    }
...
void TelemetryController::setRedisUri(const QString& uri, bool initializeFromEnv)
{
    qDebug() << "[TelemetryController] setRedisUri called with:" << uri << "initializeFromEnv:" << initializeFromEnv;
    
    // Stop telemetry client before changing configuration
    if (m_client && m_running) {
        qDebug() << "[TelemetryController] Stopping telemetry client...";
        m_client->stop();
        m_running = false;
    }
    ...
    // Update home page command client
    if (m_home) {
        m_home->setCommandRedisUri(m_redisUri);
    }
    
    // Recreate Redis facade with new URI
    try {
        qDebug() << "[TelemetryController] Recreating RedisFacade with URI:" << m_redisUri;
        m_redisFacade = std::make_unique<exoskeleton::redis::Facade>(m_redisUri.toStdString());
        
        // Initialize run:addrs from conf:env only if requested (when .env was uploaded)
        if (initializeFromEnv) {
            qDebug() << "[TelemetryController] Re-initializing run:addrs from conf:env...";
            m_redisFacade->initialize_run_addrs_from_env();
        } else {
            qDebug() << "[TelemetryController] Skipping run:addrs re-initialization (using manually created run:addrs)";
        }
        
        // Load settings from Redis
        loadRedisSettings();
        
        qDebug() << "[TelemetryController] ✓ RedisFacade recreated and configured successfully";
    } catch (const std::exception& e) {
        qWarning() << "[TelemetryController] ✗ Failed to recreate RedisFacade:" << e.what();
    }
    
    emit redisUriChanged(m_redisUri);
    
    // Restart telemetry streaming after configuration is loaded
    startTelemetry();
    m_running = true;
    
    // Trigger immediate refresh of addresses and environment from Redis
    // Use a short delay to ensure the worker thread is ready
    if (m_client) {
        qDebug() << "[TelemetryController] Scheduling Redis data refresh in 500ms...";
        QTimer::singleShot(500, this, [this]() {
            if (m_client) {
                qDebug() << "[TelemetryController] Executing scheduled Redis data refresh...";
                m_client->refreshFromRedis();
            }
        });
    }
...
void TelemetryController::loadEnvConfig()
{
    qDebug() << "[TelemetryController] Reloading configuration from Redis";
    
    if (!m_redisFacade) {
        qWarning() << "[TelemetryController] RedisFacade not initialized";
        return;
    }
    
    try {
        // Reinitialize run:addrs from conf:env
        m_redisFacade->initialize_run_addrs_from_env();
        
        // Reload settings
        loadRedisSettings();
        
        qDebug() << "[TelemetryController] Configuration reloaded successfully";
    } catch (const std::exception& e) {
        qWarning() << "[TelemetryController] Failed to reload configuration:" << e.what();
    }
}
