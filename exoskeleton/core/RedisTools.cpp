...
// Include ImuSample definition for xadd_imu_data
#include "../../include/sample.h"
    ...
    void xadd_imu_data(sw::redis::Redis &redis, int imu_id, const ImuSample &sample)
    {
        auto t_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                        sample.t_host.time_since_epoch())
                        .count();

        std::unordered_map<std::string, std::string> fields = {
            {"t_ns", std::to_string(t_ns)},
            {"seq", std::to_string(sample.seq)},
            {"imu_id", std::to_string(sample.imu_id)},
            {"accel_x", std::to_string(sample.accel[0])},
            {"accel_y", std::to_string(sample.accel[1])},
            {"accel_z", std::to_string(sample.accel[2])},
            {"gyro_x", std::to_string(sample.gyro[0])},
            {"gyro_y", std::to_string(sample.gyro[1])},
            {"gyro_z", std::to_string(sample.gyro[2])},
            {"mag_x", std::to_string(sample.mag[0])},
            {"mag_y", std::to_string(sample.mag[1])},
            {"mag_z", std::to_string(sample.mag[2])},
            {"euler_x", std::to_string(sample.euler[0])},
            {"euler_y", std::to_string(sample.euler[1])},
            {"euler_z", std::to_string(sample.euler[2])},
            {"flags", std::to_string(sample.reserved << 5 | sample.mag_ok << 2 |
                                     sample.fifo_underflow << 1 | sample.fifo_overflow)}};

        std::string stream_key = "xdata:imu:" + std::to_string(imu_id);
        redis.xadd(stream_key, "*", fields.begin(), fields.end());
    }
