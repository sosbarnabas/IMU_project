...
namespace exoskeleton::core
{
    ...
    void RedisSingleIMUController::processCommand(const std::string &raw_command)
    {
        ...
        try
        {
            ...
            else if (cmd == "icminit")
            {
                ...
                if (imu_->Initialize())
                {
                    if (imu_->FIFOConfig())
                    {
                        ...
                        log("INFO", "ICM-20948 initialized and producer started in " + std::to_string(duration.count()) + "ms");
            ...
            else if (cmd == "calibrate")
            {
                ...
                {
                    // TODO: Implement load calibration from file
        ...
        consumer_thread_ = std::thread([this]()
            ...
            try {
                while (!stop_consumer_) {
                    ...
                    fields.emplace_back("seq", std::to_string(sample_sequence_++));
                    fields.emplace_back("imu_id", std::to_string(imu_id_));
                    ...
                    fields.emplace_back("euler_x", std::to_string(euler_x));
                    fields.emplace_back("euler_y", std::to_string(euler_y));
                    fields.emplace_back("euler_z", std::to_string(euler_z));
                    uint8_t flags = 0;
    ...
    void RedisSingleIMUController::stopConsumer()
    {
        if (!consumer_running_)
            return;
        stop_consumer_ = true;
        // Wake up the consumer if it's blocked
        try
        {
